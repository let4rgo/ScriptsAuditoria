<#
	recol3ctor.ps1 - Script para Windows Poweshell >= versión 3. Este script recoge información
    del sistema usando comandos por defecto o usando Get-CmdInstance (por eso requiere ps3).
    Este script recoge información estática y dinámica
...
<#
	Nota: Para guardar el resultado en un documento de texto debe añadir "> resultado.txt"
#>
...
$auditor_company     = 'I.T.E. Ciberseguridad' # make empty string to disable
...
#############################
# Parámetros del header del documento
#############################
$script_name         = 'recol3ctor'
$script_version      = 'v1.0'
...
function Prt-ReportHeader{
    ...
    Write-Output "`n#############################"
    Write-Output "# Script de Auditoría: $script_name $script_version"
    if ($auditor_company){Write-Output "# Auditing Company: $auditor_company"}
    if ($sitename){Write-Output "# Localización / Planta: $sitename"}
    Write-Output "#############################"
...
function Prt-CutSec-ReportFooter{
    ...
    Write-Output "`n#############################"
    Write-Output "# Script de Auditoría: $script_name $script_version"
    Write-Output "# Ofrecido por I.T.E. Ciberseguridad"
    Write-Output "# Para obtener ayuda adicional,  administracion[@]ite-es.com"
    Write-Output "#############################`n"
